...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func addCharacters() {
        print("playRect is width:\(playRect.width), height:\(playRect.height)")
        
        // Add pajamas...
        for i in 1...6 {
            let pajama = Pajama(pajamaColor: (i % 2 == 0 ? PajamaColor.blue : PajamaColor.red))
            addPajama(pajama)
        }
    ...
    func addPajama(pajama: Pajama) {
        ...
        let isInRect = CGRectContainsPoint(playRect, pajama.position)
        print("Pajamas placed at X:\(randomX), Y:\(randomY). Is in rect: \(isInRect)")
    }
